...
Coming back to the initial example, we can now do a lazily cached biomart query which is robust against biomart downtime, network problems, and helps to reduce teh re-runtime of your application.
```{r}
genes <- quote({
    mart <- useDataset("drerio_gene_ensembl", mart = useMart("ensembl"))
    genes <- getBM(attributes=c("ensembl_gene_id","external_gene_name", "start_position"), mart=mart)
